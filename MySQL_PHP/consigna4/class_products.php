...
class formularioProducto
{
    ...
    public function actualizarProducto($id, $descripcion)
    {
        require 'conect.php';

        try {

            $query = "UPDATE productos SET descripcion = :descripcion WHERE ID_Producto = :id"; //Guardo la consulta en la variable query
            $stmt = $BaseSTOCK->prepare($query); //Preparo la consulta usando el metodo prepare y guardandolo en la variable "stmt" 
            $stmt->bindParam(':id', $id); //enlazo los valores recibidos por parametro (id y descripcion del formulario) con el id y descripcion de mi tabla
            $stmt->bindParam(':descripcion', $descripcion);
            $stmt->execute(); //Ejecuto la consulta
 

            $this->blAlert = true;
            $this->blUltimoAlumno = "Producto actualizado correctamente.";
        } catch (Exception $e) {
